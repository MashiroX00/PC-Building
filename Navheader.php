<nav class="navbar navbar-expand-lg navbar-dark bg-dark bg-transparent">
  <div class="container-fluid">
    <a class="navbar-brand d-flex justify-content-center align-items-center" href="<?php echo $url?>index.php">
      <img src="<?php echo $url; ?>src/assets/305582063_514973683964201_2501104888521175544_n-removebg-preview (3).png" alt="" width="62" height="62" class="d-inline-block align-text-top me-3">
      <img src="<?php echo $url; ?>src/assets/รูปโลโก้แผนก.png" alt="" width="60" height="60" class="d-inline-block align-text-top me-3">   
      <img src="<?php echo $url; ?>src/assets/RapeeMEd2-removebg-preview (1).png" alt="" width="58" height="58" class="d-inline-block align-text-top me-1">
      
      
      Basic PC Building Simulator
    </a>
    <div class="collapse navbar-collapse justify-content-end">
    <a href="<?php echo $url . "index.php"; ?>" class="btn btn-light me-2" style="width: 8%;">Home</a>
      <a href="<?php echo $url . "loginUser.php"; ?>" class="btn btn-light me-2" style="width: 8%;">Login</a>
      <a href="<?php echo $url . "register.php"; ?>" class="btn btn-light me-2" style="width: 8%;">Register</a>
    </div>
  </div>
</nav>